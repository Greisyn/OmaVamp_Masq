...
// Owns character data, renders plain text (VTM20 neonate format),
...
Item {
  ...
  readonly property string pluginId: (manifest && manifest.id) ? String(manifest.id) : "local.vampire-sheet"

  SheetConfig { id: config }

  readonly property string home: Quickshell.env("HOME")
  readonly property string characterPath: home + "/.config/omarchy/local.vampire-sheet-character.json"
  ...
  // ---- attributes: blank sheet = 1 (V20 minimum) ----
  ...
  // ---- talents (VTM20: Awareness instead of Primal-Urge) ----
  property int alertness: 0; property int athletics: 0; property int awareness: 0
  property int brawl: 0; property int empathy: 0; property int expression: 0
  property int intimidation: 0; property int leadership: 0; property int streetwise: 0
  property int subterfuge: 0
  // ---- skills (same ten as WTA20) ----
  ...
  // ---- knowledges (VTM20: Finance + Politics instead of Enigmas + Rituals) ----
  property int academics: 0; property int computer: 0; property int finance: 0
  property int investigation: 0; property int law: 0; property int medicine: 0
  property int occult: 0; property int politics: 0; property int science: 0
  property int technology: 0

  // ---- virtues (0-5, blank sheet = 1 like Attributes) / pools ----
  property int conscience: 1; property int selfControl: 1; property int courage: 1
  property int humanity: 0; property int willpower: 0
  property int bloodPool: 0
  property int experience: 0

  // ---- multiline texts ----
  property string backgrounds: ""
  property string disciplines: ""
  property string rituals: ""
  property string merits: ""
  property string flaws: ""
  property string otherTraits: ""
  ...
  function writeFile(path, text, proc) {
    ...
    var tag = "";
    do {
      tag = "__VAMPIRE_SHEET_" + Math.random().toString(36).slice(2)
  ...
  function importSheet(path) {
    var given = !(path === undefined || String(path).replace(/^\s+|\s+$/g, "") === "");
    var p = given ? String(path) : config.importPath;
    ...
    // Persist explicit paths so headless `omarchy-shell ... importSheet` reuses the last one.
    if (given) config.set("importPath", p);
